...
int main()
{
  ...
  // Same as RIA test case, using PMA instead
  ...
  // Run PMA
  PMAAlgorithm pma(rbdoProblem, solver);
  pma.setStartingPoint(Point(2, 5.0));
  pma.run();

  const Point xStar(pma.getResult().getOptimalPoint());
  std::cout << "PMA x*=" << xStar << std::endl;
